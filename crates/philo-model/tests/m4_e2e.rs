...
//! ToolRegistry + MemorySessionStore. Covers milestone M4 acceptance
//! scenarios M4-001 through M4-006; M4-007 lives in `live_smoke.rs`.
...
#[tokio::test]
async fn m4_001_multi_round_tool_loop_with_real_adapter_and_registry() {
...
#[tokio::test]
async fn m4_003_transport_failure_settles_failed_with_durable_facts() {
    let root = TempRoot::new();
...
#[tokio::test]
async fn m4_004_history_with_invalid_raw_arguments_replays_degraded() {
    let root = TempRoot::new();
...
#[tokio::test]
async fn m4_005_zero_tool_rounds_requests_without_tools() {
    let root = TempRoot::new();
...
#[tokio::test]
async fn m4_006_tool_business_errors_continue_the_loop() {
    let root = TempRoot::new();
    let transport = StubTransport::new([
