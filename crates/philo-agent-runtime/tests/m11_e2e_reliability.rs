...
#[test]
fn m11_001_crash_remnant_seals_on_disk_and_the_session_continues() {
...
#[test]
fn m11_004_placeholders_add_nothing_to_the_durable_log() {
    let root = TempRoot::new();
...
#[test]
fn m11_005_timeout_cancellation_lands_on_disk_with_its_reason() {
    let root = TempRoot::new();
    let store = Arc::new(JsonlSessionStore::open(&root.path).expect("open"));
    let gate = Gate::new();
...
#[test]
fn m11_006_legacy_cancelled_session_continues_after_upgrade() {
    let root = TempRoot::new();
